    ...
    async def attempt_to_get_sha256(self, db: Session, id: str) -> str | None:
        ...
        if sd_extra_network.safetensors:
            ...
            list_of_files_with_safetensors_name = list(
                HUB_MODELS_PATH.rglob(f"**/{sd_extra_network.safetensors.name}.safetensors")
            )
            ...
            if len(list_of_files_with_safetensors_name) > 1:
                raise ValueError(
                    f"Found multiple files with the name {sd_extra_network.safetensors.name}"
                )

        raise ValueError(f"No file found with the name {sd_extra_network.safetensors.name}")
