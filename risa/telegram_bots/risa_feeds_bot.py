...
from risa.config.config import (
    RISA_FEEDS_BOT,
    RISA_FEEDS_BOT_REPEATING_INTERVAL,
    RISA_FEEDS_BOT_TOKEN,
    TELEGRAM_CHAT_ID
)
from risa.common.constants import 
...
class RisaFeedsBot(TelegramBot, SubscriptionManager, SubscriptionWatcher):
    def init(self, updater: Updater) -> Updater:
        ...
        updater.job_queue.run_repeating(
            ...
            first=1,
            interval=RISA_FEEDS_BOT_REPEATING_INTERVAL,
        )
        return updater
    ...
    def subscribe(self, update: Update, context: CallbackContext):
        ...
        sub = self.add_subscription(
            app=RISA_FEEDS_BOT, url=args[0], chat_ids=[TELEGRAM_CHAT_ID]
        )
        update.message.reply_text(
            text=f"Sucessfuly added new subscription. Will get 3 latest posts... \n{args=}"
        )
        self.check_subscription_for_updates(app=RISA_FEEDS_BOT, sub=sub, context=context)

    def list(self, update: Update, context: CallbackContext):
        update.message.reply_text(text=self.get_subscriptions_as_text(app=RISA_FEEDS_BOT))
    ...
    def remove(self, update: Update, context: CallbackContext):
        if not (args := self.get_args_from_message(update=update, context=context)):
            return
        url_key = args[0]
        self.remove_subscription(app=RISA_FEEDS_BOT, url_key=url_key)
    ...
    def repeating_check_boards_for_updates(self, context: CallbackContext):
        return self.check_app_for_updates(app=RISA_FEEDS_BOT, context=context)
